...
export async function deleteThought(id: string, path: string): Promise<void> {
  try {
    ...
    const mainThought = await Thought.findById(id).populate("author group");
    ...
    const uniqueGroupIds = new Set(
      [
        ...descendantThoughts.map((thought) => thought.group?._id?.toString()), // Use optional chaining to handle possible undefined values
        mainThought.group?._id?.toString(),
      ].filter((id) => id !== undefined)
    );
